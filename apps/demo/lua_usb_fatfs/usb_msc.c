...
{
    return (write_generation != processed_generation) &&
           (ejected || ((now_ms - last_write_ms) >= idle_timeout_ms));
}

void lua_usb_msc_mark_processed(void)
{
    processed_generation = write_generation;
}
...
{
    LOG_INFO("usb", "Lua script disk mounted");
}

void tud_umount_cb(void)
{
    LOG_INFO("usb", "Lua script disk unmounted");
}
                             ...
                             uint32_t buffer_size)
{
    ...
    static const char product[] = "Lua Script Disk";
                        ...
                        uint16_t buffer_size)
{
    ...
    (void)buffer_size;
    if (command[0] == 0x35U) {
        return (ota_spi_disk_sync(msc_disk) == CHIP_OK) ? 0 : -1;
    }
