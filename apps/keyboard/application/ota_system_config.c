...
#define CONFIG_FIELD_DISK    UINT8_C(0x20)
#define CONFIG_FIELDS_ALL    UINT8_C(0x3F)
...
void ota_system_config_defaults(ota_system_config_t *config)
{
    ...
    memcpy(config->device_name, "Mosaic Keyboard", sizeof("Mosaic Keyboard"));
    config->usb_disk_visible = true;
    config->log_level = PLATFORM_LOG_LEVEL_DEBUG;
...
static bool parse_bool(const char *json, const jsmntok_t *token, bool *value)
{
    if ((token->type != JSMN_PRIMITIVE) || (value == NULL)) {
        return false;
    }
    if (token_equals(json, token, "true")) {
        *value = true;
    } else if (token_equals(json, token, "false")) {
        *value = false;
    } else {
        return false;
    }
    ...
    ota_system_config_t *config, uint8_t *fields)
{
    ...
    } else if (token_equals(json, key, "usb_disk_visible")) {
        field = CONFIG_FIELD_DISK;
        valid = parse_bool(json, value, &config->usb_disk_visible);
...
static ota_system_config_status_t write_config_file(const char *json,
                                                    size_t size)
{
    FIL file;
    UINT written = 0U;
    ...
    result = f_write(&file, json, (UINT)size, &written);
    ...
    result = f_rename(OTA_SYSTEM_CONFIG_TEMP_PATH, OTA_SYSTEM_CONFIG_PATH);
    if (result == FR_EXIST) {
        result = f_unlink(OTA_SYSTEM_CONFIG_PATH);
        if (result == FR_OK) {
            result = f_rename(OTA_SYSTEM_CONFIG_TEMP_PATH,
                              OTA_SYSTEM_CONFIG_PATH);
        }
    }
    if (result != FR_OK) {
        (void)f_unlink(OTA_SYSTEM_CONFIG_TEMP_PATH);
...
{
    ota_system_config_status_t status = write_config_file(
        default_json, sizeof(default_json) - 1U);

    return (status == OTA_SYSTEM_CONFIG_OK) ? OTA_SYSTEM_CONFIG_CREATED :
                                             status;
}
    ...
    const char *json, size_t size, ota_system_config_t *config)
{
    ...
    if (size > OTA_SYSTEM_CONFIG_MAX_FILE_SIZE) {
        return OTA_SYSTEM_CONFIG_TOO_LARGE;
    }
    status = ota_system_config_parse(json, size, &parsed);
    if (status != OTA_SYSTEM_CONFIG_OK) {
        return status;
    }
    mount_result = f_mount(&config_filesystem, "0:", 1U);
    if (mount_result != FR_OK) {
        return OTA_SYSTEM_CONFIG_MOUNT_ERROR;
    }
    status = write_config_file(json, size);
    (void)f_unmount("0:");
    if (status == OTA_SYSTEM_CONFIG_OK) {
        *config = parsed;
    }
