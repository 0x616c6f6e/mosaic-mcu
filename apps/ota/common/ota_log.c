...
static int ota_log_backend(const char *data, size_t size, void *context)
{
    size_t written = 0U;
...
static uint32_t ota_log_timestamp(void *context)
{
    (void)context;
    return chip_time_millis();
}

chip_status_t ota_log_init(void)
{
    ...
    const platform_log_config_t log_config = {
        .backend = ota_log_backend,
        .backend_context = NULL,
        .timestamp = ota_log_timestamp,
...
chip_status_t ota_log_flush(void)
{
