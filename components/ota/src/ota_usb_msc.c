...
static bool ejected;
static char inquiry_vendor[8] = "Mosaic";
static char inquiry_product[16] = "OTA Disk";
static char inquiry_revision[4] = "1.0";

static bool copy_inquiry_field(char *destination, size_t capacity,
                               const char *source)
{
    size_t length;
...
void ota_usb_msc_init(ota_spi_disk_t *disk)
{
    ...
    ejected = false;
    LOG_DEBUG("usb", "MSC ready sectors=%lu sector_size=%lu",
              (unsigned long)ota_spi_disk_sector_count(disk),
              (unsigned long)OTA_DISK_SECTOR_SIZE);
}
...
chip_status_t ota_usb_msc_set_identity(const char *vendor_id,
                                       ...
                                       const char *revision)
{
    char vendor[sizeof(inquiry_vendor)];
    char product[sizeof(inquiry_product)];
    char version[sizeof(inquiry_revision)];

    if (!copy_inquiry_field(vendor, sizeof(vendor), vendor_id) ||
        !copy_inquiry_field(product, sizeof(product), product_id) ||
        !copy_inquiry_field(version, sizeof(version), revision)) {
        return CHIP_ERROR_INVALID_ARG;
    }
    memcpy(inquiry_vendor, vendor, sizeof(inquiry_vendor));
    memcpy(inquiry_product, product, sizeof(inquiry_product));
    memcpy(inquiry_revision, version, sizeof(inquiry_revision));
    return CHIP_OK;
}
...
void ota_usb_msc_mark_checked(void)
{
    ...
    LOG_DEBUG("usb", "write generation %lu checked",
              (unsigned long)checked_generation);
}

void ota_usb_msc_resume(void)
{
    ejected = false;
    LOG_DEBUG("usb", "MSC media resumed");
}
                             ...
                             uint32_t buffer_size)
{
    (void)lun;
    (void)buffer_size;
    memcpy(response->vendor_id, inquiry_vendor, sizeof(inquiry_vendor));
    memcpy(response->product_id, inquiry_product, sizeof(inquiry_product));
    memcpy(response->product_rev, inquiry_revision,
           sizeof(inquiry_revision));
                         ...
                         uint16_t *block_size)
{
    ...
    *block_size = (uint16_t)OTA_DISK_SECTOR_SIZE;
    LOG_DEBUG("usb", "capacity requested blocks=%lu block_size=%u",
              (unsigned long)*block_count, (unsigned int)*block_size);
}
                           ...
                           bool load_eject)
{
    ...
    LOG_DEBUG("usb", "start-stop start=%u eject=%u",
              start ? 1U : 0U, load_eject ? 1U : 0U);
    if (load_eject && !start) {
        ejected = true;
                           ...
                           uint8_t *buffer, uint32_t buffer_size)
{
    ...
    if (write_generation == checked_generation) {
        LOG_DEBUG("usb", "host write started lba=%lu offset=%lu size=%lu",
                  (unsigned long)lba, (unsigned long)offset,
                  (unsigned long)buffer_size);
    }
    last_write_ms = chip_time_millis();
                        ...
                        uint16_t buffer_size)
{
    ...
    if (command[0] == 0x35U) {
        if (ota_spi_disk_sync(msc_disk) == CHIP_OK) {
            LOG_DEBUG("usb", "SCSI synchronize-cache complete");
            return 0;
        }
        LOG_ERROR("usb", "SCSI synchronize-cache failed");
        return -1;
    }
    LOG_DEBUG("usb", "unsupported SCSI command=0x%02X",
